...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-dataproc-logging.git --from 0.0.0-preview
```

Then add `GoogleCloudDataprocLogging` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudDataprocLogging <target-name> --package swift-google-cloud-dataproc-logging
```
